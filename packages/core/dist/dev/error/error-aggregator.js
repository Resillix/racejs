...
export class ErrorAggregator extends EventEmitter {
     ...
     */
    constructor(storage, maxErrors = 1000, maxOccurrences = 100) {
        super();
        this.errors = new Map();
        if (storage) {
    ...
    listErrors(filter) {
        ...
        if (filter.search) {
            ...
            errors = errors.filter((e) => e.message.toLowerCase().includes(query) || e.type.toLowerCase().includes(query));
        }
        return errors;
    }
    /**
    ...
    exportErrors(format = 'json') {
        const errors = Array.from(this.errors.values());
        if (format === 'json') {
            return JSON.stringify(errors, (_key, value) => {
