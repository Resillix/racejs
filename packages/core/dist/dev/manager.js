...
export class DevModeManager extends EventEmitter {
    ...
    constructor(options = {}) {
        ...
        if (this.options.recorder) {
            this.recorder = new RequestRecorder(typeof this.options.recorder === 'object' ? this.options.recorder : undefined);
        }
        // Initialize profiler if enabled
        if (this.options.profiler) {
            const profilerOptions = typeof this.options.profiler === 'object' ? this.options.profiler : {};
        ...
        this._startPromise = (async () => {
            try {
                ...
                if (this.options.devtools) {
                    ...
                    const devtoolsOptions = typeof this.options.devtools === 'object' ? this.options.devtools : {};
     ...
     */
    setServerInfo(port, host = 'localhost') {
        if (this.recorder) {
     ...
     * Record a request
     */
    recordRequest(route, method, duration, isError = false) {
        ...
        return async (req, res, next) => {
            ...
            try {
                ...
                if (requestId) {
                    ...
                    req.__devModeRequestId = requestId;
                    // Intercept response
        ...
        return async (err, req, res, _next) => {
            ...
            try {
                ...
                this.errorAggregator.track(err, {
                    ...
                    timestamp: Date.now(),
                    requestId: req.__devModeRequestId,
 ...
 */
export function createDevMode(options) {
    const normalized = normalizeDevModeOptions(options);
    if (!normalized)
        return null;
