...
export class DevToolsMessageHandler {
    ...
    setupEventListeners() {
        ...
        if (recorder) {
            recorder.on('recorded', async ({ id, request }) => {
                this.server.broadcast(createServerMessage(ServerMessageType.REQUEST_RECORDED, {
                    id,
            ...
            recorder.on('response-recorded', async ({ id, request }) => {
                this.server.broadcast(createServerMessage(ServerMessageType.REQUEST_RESPONSE, {
                    id,
        ...
        if (errorAggregator) {
            ...
            errorAggregator.on('error-tracked', ({ hash, error }) => {
                this.server.broadcast(createServerMessage(ServerMessageType.ERROR_TRACKED, {
                    hash: hash,
                    name: error.type,
            ...
            errorAggregator.on('error-spike', (error) => {
                this.server.broadcast(createServerMessage(ServerMessageType.ERROR_SPIKE_ALERT, {
                    hash: error.hash,
                    message: error.message,
                    count: error.count,
                    trend: error.trend,
        ...
        try {
            switch (message.type) {
                ...
                case ClientMessageType.EXPORT_ERRORS:
                    ...
                    break;
                default:
                    this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
     ...
     */
    async handleGetMetrics(client) {
        const metrics = this.devMode.getMetrics();
        const memory = process.memoryUsage();
        this.server.send(client, createServerMessage(ServerMessageType.METRICS_UPDATE, {
     ...
     */
    async handleGetRoutes(client) {
        // Note: We'll need to add a method to Application to get routes
        // For now, send empty array
        this.server.send(client, createServerMessage(ServerMessageType.ROUTES_LIST, {
            routes: [],
        }));
    }
    /**
     ...
     */
    async handleGetRequests(client, data) {
        const recorder = this.devMode.getRecorder();
        if (!recorder) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
        ...
        this.server.send(client, createServerMessage(ServerMessageType.REQUESTS_LIST, {
            total,
     ...
     */
    async handleGetRequest(client, data) {
        const recorder = this.devMode.getRecorder();
        if (!recorder) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Recorder not enabled',
     ...
     */
    async handleClearRequests(_client) {
        const recorder = this.devMode.getRecorder();
        if (!recorder) {
            return;
        }
        ...
        this.server.broadcast(createServerMessage(ServerMessageType.REQUESTS_LIST, {
            total: 0,
            requests: [],
        }));
    }
     ...
     */
    async handleDeleteRequest(client, data) {
        const recorder = this.devMode.getRecorder();
        if (!recorder) {
            return;
        }
     ...
     */
    async handleExportRequests(client) {
        const recorder = this.devMode.getRecorder();
        if (!recorder) {
            return;
        }
        ...
        // Send the JSON data
        this.server.send(client, createServerMessage(ServerMessageType.REQUESTS_EXPORT, {
            data: json,
        }));
    }
    /**
    ...
    broadcastMetrics() {
        ...
        const metrics = this.devMode.getMetrics();
        const memory = process.memoryUsage();
        this.server.broadcast(createServerMessage(ServerMessageType.METRICS_UPDATE, {
            requests: metrics.totalRequests,
            errors: metrics.totalErrors,
            avgResponseTime: metrics.avgResponseTime,
            memory: {
     ...
     */
    async handleReplayRequest(client, data) {
        const recorder = this.devMode.getRecorder();
        if (!recorder) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Recorder not enabled',
        ...
        try {
            ...
            this.server.send(client, createServerMessage(ServerMessageType.REPLAY_RESULT, {
                success: true,
                originalId: data.id,
                ...
                response: {
                    ...
                    headers: result.replayResponse.headers,
     ...
     */
    async handleCompareResponses(client, data) {
        const recorder = this.devMode.getRecorder();
        if (!recorder) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Recorder not enabled',
        ...
        try {
            ...
            const comparison = replayEngine.compareResponses(originalRequest.response, replayRequest.response);
            this.server.send(client, createServerMessage(ServerMessageType.REPLAY_COMPARISON, {
     ...
     */
    async handleGenerateTest(client, data) {
        const recorder = this.devMode.getRecorder();
        if (!recorder) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Recorder not enabled',
        ...
        try {
            ...
            this.server.send(client, createServerMessage(ServerMessageType.TEST_GENERATED, {
     ...
     */
    async handleEditAndReplay(client, data) {
        const recorder = this.devMode.getRecorder();
        if (!recorder) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Recorder not enabled',
     ...
     */
    async handleGetPerformanceMetrics(client) {
     ...
     */
    async handleStartProfiling(client) {
        ...
        try {
            ...
            this.server.send(client, createServerMessage(ServerMessageType.PROFILING_STARTED, {
                startTime: Date.now(),
            }));
        }
        catch (error) {
     ...
     */
    async handleStopProfiling(client) {
        const profiler = this.devMode.getProfiler();
        if (!profiler) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Performance profiler is not enabled',
        ...
        try {
            ...
            this.server.send(client, createServerMessage(ServerMessageType.PROFILING_STOPPED, {
                endTime: Date.now(),
     ...
     */
    async handleGetErrors(client, data) {
        const errorAggregator = this.devMode.getErrorAggregator();
        if (!errorAggregator) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
        ...
        try {
            ...
            this.server.send(client, createServerMessage(ServerMessageType.ERROR_LIST, {
                errors: errorList,
     ...
     */
    async handleGetErrorDetails(client, data) {
        const errorAggregator = this.devMode.getErrorAggregator();
        if (!errorAggregator) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Error aggregator is not available',
        ...
        try {
            ...
            this.server.send(client, createServerMessage(ServerMessageType.ERROR_DETAILS, {
                hash: error.hash,
                message: error.message,
                type: error.type,
                stack: error.stack,
     ...
     */
    async handleGetErrorStats(client) {
        const errorAggregator = this.devMode.getErrorAggregator();
        if (!errorAggregator) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Error aggregator is not available',
        ...
        try {
            ...
            this.server.send(client, createServerMessage(ServerMessageType.ERROR_STATS, {
                totalErrors,
                uniqueErrors,
                errorRate,
                criticalErrors,
     ...
     */
    async handleMarkErrorResolved(client, data) {
        const errorAggregator = this.devMode.getErrorAggregator();
        if (!errorAggregator) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Error aggregator is not available',
     ...
     */
    async handleMarkErrorIgnored(client, data) {
        const errorAggregator = this.devMode.getErrorAggregator();
        if (!errorAggregator) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Error aggregator is not available',
     ...
     */
    async handleClearErrors(client) {
        const errorAggregator = this.devMode.getErrorAggregator();
        if (!errorAggregator) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Error aggregator is not available',
     ...
     */
    async handleExportErrors(client, data) {
        const errorAggregator = this.devMode.getErrorAggregator();
        if (!errorAggregator) {
            this.server.send(client, createServerMessage(ServerMessageType.ERROR, {
                message: 'Error aggregator is not available',
...
//# sourceMappingURL=devtools-handler.js.map