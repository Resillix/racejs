...
import { EnhancedError, SourceContext, Solution } from './types';
...
export class ErrorRenderer {
  private options: ErrorRendererOptions;
...
<body class="theme-${theme}">
   ...
   */
  private renderHeader(error: EnhancedError): string {
   ...
   */
  private renderTabContent(error: EnhancedError): string {
    return `
   ...
   */
  private renderSourceCodeTab(error: EnhancedError): string {
   ...
   */
  private renderSourceContext(context: SourceContext): string {
   ...
   */
  private renderStackTraceTab(error: EnhancedError): string {
   ...
   */
  private renderStackLine(line: string): string {
   ...
   */
  private renderSolutionsTab(error: EnhancedError): string {
   ...
   */
  private renderSolutionCard(solution: Solution): string {
    const confidencePercent = Math.round(solution.confidence * 100);
    const confidenceClass =
      solution.confidence > 0.8 ? 'high' : solution.confidence > 0.5 ? 'medium' : 'low';
   ...
   */
  private renderRequestTab(error: EnhancedError): string {
   ...
   */
  private renderFooter(error: EnhancedError): string {
    return `
   ...
   */
  private renderFileLink(file: string, line: number): string {
   ...
   */
  private getEditorUrl(file: string, line: number): string {
   ...
   */
  private getErrorIcon(statusCode: number): string {
   ...
   */
  private highlightCode(code: string, _language: string): string {
   ...
   */
  private getCSS(theme: 'dark' | 'light'): string {
    return `
    * {
      margin: 0;
      padding: 0;
