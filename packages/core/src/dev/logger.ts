...
export class DevLogger {
  private options: Required<DevLoggerOptions>;
...
export class ConsoleTransport implements LogTransport {
  log(entry: LogEntry): void {
    ...
    if (context.requestId) contextParts.push(`req:${context.requestId.slice(0, 8)}`);
    if (context.method && context.route) contextParts.push(`${context.method} ${context.route}`);
 ...
 */
export function createDevLogger(options?: DevLoggerOptions): DevLogger {
